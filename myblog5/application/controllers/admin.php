...
class Admin extends CI_Controller {
    ...
    public function logout(){
        $this -> session -> unset_userdata('admin');//登出时清空sesssion中的admin
    ...
    public  function admin_mgr(){
        ...
        $config['total_rows'] = $this -> admin_model -> get_admin_count();//get_admin_count()方法，获取总共的记录数
        $config['per_page'] = 15;
...
//        }
	...
	public function  delete_blogs(){
        $blog_ids = $this -> input -> get('blog_ids');
		$this -> load -> model('blog_model');
        $this -> blog_model -> delete_blogs($blog_ids);
        $this -> blog_mgr();
    }
